...
class Document:
    ...
    async def find_one(self, id):
        ...
        Custom find
        """
        return await self.db.find_one(id)
    ...
    async def find_more(self, obj):
        """
        For simpler calls, points to self.find_by_id
        """
        data = []
        async for document in self.db.find(obj):
            data.append(document)
        return data
    ...
    async def get_all_id(self):
        ...
        data = {}
        async for document in self.db.find({}):
            smallerDocument =  copy.deepcopy(document)
            del smallerDocument["_id"]
            data[document["_id"]] = smallerDocument
        return data